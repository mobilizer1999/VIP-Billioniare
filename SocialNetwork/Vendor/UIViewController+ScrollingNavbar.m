...
- (void)scrollWithDelta:(CGFloat)delta
{
	...
	if (delta < 0) {
		...
		frame.origin.y = MIN([UIApplication sharedApplication].statusBarFrame.size.height, frame.origin.y - delta);
		self.navigationController.navigationBar.frame = frame;
...
{
    UIScrollView* scroll;
    if ([self.scrollableView isKindOfClass:[WKWebView class]]) {
        scroll = [(WKWebView*)self.scrollableView scrollView];
