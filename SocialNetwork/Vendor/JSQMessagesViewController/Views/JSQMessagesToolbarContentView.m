...
@property (weak, nonatomic) IBOutlet UIView *middleRightBarButtonContainerView;
...
- (void)setLeftBarButtonItem1 :(UIButton *)leftBarButtonItem
{
    if (_leftBarButtonItem1) {
        [_leftBarButtonItem1 removeFromSuperview];
    }
    
    if (!leftBarButtonItem) {
        _leftBarButtonItem1 = nil;
        self.middleBarButtonContainerView.hidden = YES;
        return;
    }
    
    if (CGRectEqualToRect(leftBarButtonItem.frame, CGRectZero)) {
        leftBarButtonItem.frame = self.middleBarButtonContainerView.bounds;
    }
    ...
    [self.middleBarButtonContainerView addSubview:leftBarButtonItem];
    [self.middleBarButtonContainerView jsq_pinAllEdgesOfSubview:leftBarButtonItem];
    [self setNeedsUpdateConstraints];
    
    _leftBarButtonItem1 = leftBarButtonItem;
}



- (void)setLeftBarButtonItem2 :(UIButton *)leftBarButtonItem
{
    if (_leftBarButtonItem2) {
        [_leftBarButtonItem2 removeFromSuperview];
    }
    
    if (!leftBarButtonItem) {
        _leftBarButtonItem2 = nil;
        self.middleRightBarButtonContainerView.hidden = YES;
        return;
    }
    
    if (CGRectEqualToRect(leftBarButtonItem.frame, CGRectZero)) {
        leftBarButtonItem.frame = self.middleRightBarButtonContainerView.bounds;
    }
    
    self.middleRightBarButtonContainerView.hidden = NO;
    self.leftBarButtonItemWidth = CGRectGetWidth(leftBarButtonItem.frame);
    
    [self.middleRightBarButtonContainerView addSubview:leftBarButtonItem];
    [self.middleRightBarButtonContainerView jsq_pinAllEdgesOfSubview:leftBarButtonItem];
    [self setNeedsUpdateConstraints];
    
    _leftBarButtonItem2 = leftBarButtonItem;
}
...
{
    self.leftBarButtonContainerViewWidthConstraint.constant = 0; ///leftBarButtonItemWidth;
    [self setNeedsUpdateConstraints];
}
