    ...
    [queryExistingLikes findObjectsInBackgroundWithBlock:^(NSArray *activities, NSError *error) {
        ...
        [likeActivity saveInBackgroundWithBlock:^(BOOL succeeded, NSError *error) {
            ...
            [query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error) {
                if (!error) {
                    ...
                    for (PFObject *activity in objects) {
                        if ([[activity objectForKey:kESActivityTypeKey] isEqualToString:kESActivityTypeLikePhoto] && [activity objectForKey:kESActivityFromUserKey]) {
                            senderPushObject = [activity objectForKey:kESActivityFromUserKey];

                            [likers addObject:[activity objectForKey:kESActivityFromUserKey]];
                        }  if ([[activity objectForKey:kESActivityTypeKey] isEqualToString:kESActivityTypeLikePhoto] && [activity objectForKey:kESActivityToUserKey]) {
                            
                            recieverPushObject = [activity objectForKey:kESActivityToUserKey];
                ...
                [self deliverPushOfPostLike:senderPushObject recievePushUser:recieverPushObject];
                ...
                if (!error) {
                    ...
                    for (PFObject *activity in objects) {
                        if ([[activity objectForKey:kESActivityTypeKey] isEqualToString:kESActivityTypeLikePost] && [activity objectForKey:kESActivityFromUserKey]) {
                            ...
                            [likers addObject:[activity objectForKey:kESActivityFromUserKey]];
                        }  if ([[activity objectForKey:kESActivityTypeKey] isEqualToString:kESActivityTypeLikePost] && [activity objectForKey:kESActivityToUserKey]) {
            ...
            [query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error) {
                if (!error) {
                    ...
                    for (PFObject *activity in objects) {
                        ...
                        if ([[activity objectForKey:kESActivityTypeKey] isEqualToString:kESActivityTypeLikeVideo] && [activity objectForKey:kESActivityToUserKey]) {
...
+ (void)processProfilePictureData:(NSData *)newProfilePictureData {
  ...
  /*  UIImage *resizedImage = [image resizedImageWithContentMode:UIViewContentModeScaleAspectFit bounds:CGSizeMake(640.0f, 640.0f) interpolationQuality:kCGInterpolationHigh];
    ...
    NSData *mediumImageData = UIImageJPEGRepresentation(resizedImage, 0.8f);
    NSData *smallRoundedImageData = UIImagePNGRepresentation(thumbnailImage);*/
...
+ (void)followUserInBackground:(PFUser *)user block:(void (^)(BOOL succeeded, NSError *error))completionBlock {
    if ([[user objectId] isEqualToString:[[PFUser currentUser] objectId]]) {
     ...
     __block PFUser *senderPushObject = [PFUser currentUser];;
    __block PFUser *recieverPushObject = user;
    ...
    [followActivity saveInBackgroundWithBlock:^(BOOL succeeded, NSError *error) {
        if (completionBlock) {
            ...
            if (!error) {
                ...
                [self deliverPushOfFollowUser:senderPushObject recievePushUser:recieverPushObject];
...
+ (void)followUserEventually:(PFUser *)user block:(void (^)(BOOL succeeded, NSError *error))completionBlock {
     PFUser *senderPushObject = [PFUser currentUser];
     PFUser *recieverPushObject = user;
         ...
         if (snapshot.value != [NSNull null])
         {
             ...
             if (recent != nil)
             {
                ...
                /* PFQuery *query = [PFQuery queryWithClassName:kESUserClassNameKey];
                 ...
                 [queryInstallation whereKey:@"user" equalTo:@"2CVYr0c69z"];

                 [queryInstallation whereKey:kESInstallationUserKey matchesQuery:query];
                 
                 [queryInstallation findObjectsInBackgroundWithBlock:^(NSArray * _Nullable objects, NSError * _Nullable error) {
                 
                     if (!error) {
                         NSLog(@"%@",objects);
                     } else {
                         NSLog(@"Unable to retrieve object with title %@",error.description);
                     }
                ...
                /* NSDictionary *data = [NSDictionary dictionaryWithObjectsAndKeys:
                 ...
                 NSMutableArray *array = recent[@"members"];
                 
                 if ([array containsObject:user.objectId])
                 {
                     [array removeObject:user.objectId];
                 }
                 
                 NSLog(@"user.objectId %@",user.objectId);
                 
                 NSString *strReciever = @"";
                 
                 if ([array count]>0) {
                     
                     strReciever = [array objectAtIndex:0];

                 }
                 
                 NSLog(@"%@",strReciever);
                 
                 NSDictionary *paramaeter = [NSDictionary dictionaryWithObjectsAndKeys:
                                       message, @"message",
                                       strReciever, @"recieveId",
...
+(void)deliverPushOfPostLike:(PFObject*)sentPushUser recievePushUser:(PFObject*)recievePushUser
{
    PFUser *user = (PFUser *)sentPushUser;
    
    NSString *message = [NSString stringWithFormat:@"%@ liked your post",[user objectForKey:@"displayName"]];
    
    
    NSDictionary *paramaeter = [NSDictionary dictionaryWithObjectsAndKeys:
                                message, @"message",
                                recievePushUser.objectId, @"recieveId",
                                sentPushUser.objectId, @"userId",
                                nil];
    //                                       @"2CVYr0c69z", @"userId",
    ...
    [PFCloud callFunctionInBackground:@"sendLikePostPush"
                       withParameters:paramaeter
                                block:^(id object, NSError *error) {
                                    if (!error) {
                                        NSLog(@"PUSH SENT");
...
+(void)deliverPushOfPostComment:(PFObject*)sentPushUser recievePushUser:(PFObject*)recievePushUser
{
    PFUser *user = (PFUser *)sentPushUser;
    
    NSString *message = [NSString stringWithFormat:@"%@ commented your post",[user objectForKey:@"displayName"]];
    ...
    [PFCloud callFunctionInBackground:@"sendCommentPostPush"
                       withParameters:paramaeter
                                block:^(id object, NSError *error) {
                                    if (!error) {
                                        NSLog(@"PUSH SENT");
...
+(void)deliverPushOfFollowUser:(PFUser*)sentPushUser recievePushUser:(PFUser*)recievePushUser
{
    
    NSString *message = [NSString stringWithFormat:@"%@ follow you",[sentPushUser objectForKey:@"displayName"]];
    ...
    [PFCloud callFunctionInBackground:@"sendFollowPostPush"
                       withParameters:paramaeter
                                block:^(id object, NSError *error) {
                                    if (!error) {
                                        NSLog(@"PUSH SENT");
...
+ (NSString *)createConversation:(NSMutableArray *)users groupName:(NSString *)groupName image:(UIImage*)image
{
    NSString *groupId = @"";
    ...
    if ([users count] == 2)
    {
        ...
        [self createConversationItemWitUser:user1 groupId:groupId members:userIds andDescription:[user2 objectForKey:kESUserDisplayNameKey] image:image];
        [self createConversationItemWitUser:user2 groupId:groupId members:userIds andDescription:[user1 objectForKey:kESUserDisplayNameKey] image:image];
        return groupId;
    }
    ...
    groupId = [groupId stringByAppendingString:timestamp];
    ...
    for (PFUser *user in users)
    {
        [self createConversationItemWitUser:user groupId:groupId members:userIds andDescription:description image:image];
    }
    return groupId;
}

+ (void)createConversationItemWitUser:(PFUser *)user groupId:(NSString *)groupId members:(NSArray *)members andDescription:(NSString *)description image:(UIImage*)image
{
    ...
    [query observeSingleEventOfType:FEventTypeValue withBlock:^(FDataSnapshot *snapshot)
     {
         ...
         if (create) {
            ...
            // NSDictionary *recent;
             // recent = @{@"recentId":recentId, @"userId":user.objectId, @"groupId":groupId, @"members":members, @"description":description, @"lastUser":lastUser.objectId, @"lastMessage":@"", @"counter":@0, @"date":date};

             NSMutableDictionary *messengerItem = [[NSMutableDictionary alloc] init];

             [messengerItem setObject:recentId forKey:@"recentId"];
             [messengerItem setObject:user.objectId forKey:@"userId"];
             [messengerItem setObject:groupId forKey:@"groupId"];
             [messengerItem setObject:members forKey:@"members"];
             [messengerItem setObject:description forKey:@"description"];
             [messengerItem setObject:lastUser.objectId forKey:@"lastUser"];
             [messengerItem setObject:@"" forKey:@"lastMessage"];
             [messengerItem setObject:@0 forKey:@"counter"];
             [messengerItem setObject:date forKey:@"date"];

             if ([members count]>2) {
                 
                 if (image == nil) {
                     
                     [reference setValue:messengerItem withCompletionBlock:^(NSError *error, Firebase *ref)
                      {
                 ...
                 {
                     PFFile *file = [PFFile fileWithName:@"picture.jpg" data:UIImageJPEGRepresentation(image, 0.01)];
                     
                     [file saveInBackgroundWithBlock:^(BOOL succeeded, NSError *error) {
                         if (error == nil) {
                             [messengerItem setObject:file.url forKey:@"groupIcon"];
                             [ProgressHUD dismiss];
...
+ (void)updateConversationItemWitUser:(NSString *)userId groupId:(NSString *)groupId members:(NSArray *)members andDescription:(NSString *)description image:(UIImage*)image recentId:(NSString *)recentId
{
    Firebase *firebase = [[Firebase alloc] initWithUrl:[NSString stringWithFormat:@"%@/Conversations", kESChatFirebaseCredentialKey]];
    ...
    [query observeSingleEventOfType:FEventTypeValue withBlock:^(FDataSnapshot *snapshot)
     {
             ...
             Firebase *reference = [firebase childByAppendingPath:recentId];
             PFUser *lastUser = [PFUser currentUser];
             NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
             [formatter setDateFormat:@"yyyy'-'MM'-'dd'T'HH':'mm':'ss'.'zzz'"];
             [formatter setTimeZone:[NSTimeZone timeZoneForSecondsFromGMT:0]];
             ...
             [messengerItem setObject:recentId forKey:@"recentId"];
             [messengerItem setObject:userId forKey:@"userId"];
             [messengerItem setObject:groupId forKey:@"groupId"];
             [messengerItem setObject:members forKey:@"members"];
             [messengerItem setObject:description forKey:@"description"];
             [messengerItem setObject:lastUser.objectId forKey:@"lastUser"];
             ...
             if ([members count]>2) {
                 ...
                 if (image == nil) {
                     ...
                     [reference setValue:messengerItem withCompletionBlock:^(NSError *error, Firebase *ref)
                      {
                          if (error != nil) NSLog(@"updateConversation save error.");
                      }];
                 }
                 else
                 {
...
+ (void)removeConversationItemWitUser:(PFUser *)user groupId:(NSString *)groupId members:(NSArray *)members andDescription:(NSString *)description image:(UIImage*)image recentId:(NSString *)recentId
{
    Firebase *firebase = [[Firebase alloc] initWithUrl:[NSString stringWithFormat:@"%@/Conversations", kESChatFirebaseCredentialKey]];
    ...
    [query observeSingleEventOfType:FEventTypeValue withBlock:^(FDataSnapshot *snapshot)
     {
         ...
         [messengerItem setObject:recentId forKey:@"recentId"];
         [messengerItem setObject:lastUser.objectId forKey:@"userId"];
         [messengerItem setObject:groupId forKey:@"groupId"];
         [messengerItem setObject:members forKey:@"members"];
         [messengerItem setObject:description forKey:@"description"];
         [messengerItem setObject:lastUser.objectId forKey:@"lastUser"];
         ...
         if ([members count]>2) {
             ...
             if (image == nil) {
                 ...
                 [reference updateChildValues:messengerItem withCompletionBlock:^(NSError *error, Firebase *ref)
                  {
                      if (error != nil) NSLog(@"updateConversation save error.");
                  }];
             }
...
+ (BOOL)shouldPresentPhotoCamera:(id)target editable:(BOOL)editable
{
...
+ (BOOL)shouldPresentPhotoAndVideoLibrary:(id)target editable:(BOOL)editable
{
    if ([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypePhotoLibrary] == NO) return NO;
    NSString *type1 = (NSString *)kUTTypeImage;
    NSString *type2 = (NSString *)kUTTypeMovie;
    UIImagePickerController *imagePicker = [[UIImagePickerController alloc] init];
    if ([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypePhotoLibrary]
        && [[UIImagePickerController availableMediaTypesForSourceType:UIImagePickerControllerSourceTypePhotoLibrary] containsObject:type1])
    {
        imagePicker.mediaTypes = @[type1, type2];
...
+ (NSString *)calculateElapsedPeriod:(NSTimeInterval)seconds
{
    ...
    else if (seconds < 60 * 60)
    {
        ...
        elapsed = [NSString stringWithFormat:@"%d %@", minutes, (minutes > 1) ? NSLocalizedString(@"mins", nil) : NSLocalizedString(@"min", nil)];
    }
    else if (seconds < 24 * 60 * 60)
    {
        int hours = (int) (seconds / (60 * 60));
        elapsed = [NSString stringWithFormat:@"%d %@", hours, (hours > 1) ? NSLocalizedString(@"hours", nil) : NSLocalizedString(@"hour", nil)];
    }
    else
    {
        int days = (int) (seconds / (24 * 60 * 60));
        elapsed = [NSString stringWithFormat:@"%d %@", days, (days > 1) ? NSLocalizedString(@"days", nil) : NSLocalizedString(@"day", nil)];
    }
    return elapsed;
}
