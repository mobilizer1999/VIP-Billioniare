...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
       ...
       // [self.activityImageView setBackgroundColor:[UIColor redColor]];
        ...
        CGRect nameButtonFrame = self.nameButton.frame;
        nameButtonFrame.origin.y = 16;
        self.nameButton.frame = nameButtonFrame;
...
- (void)layoutSubviews {
    ...
    [self.activityImageView setFrame:CGRectMake( [UIScreen mainScreen].bounds.size.width - 50.0f, 5.0f, 40.0f, 40.0f)];
    [self.activityImageButton setFrame:CGRectMake( [UIScreen mainScreen].bounds.size.width - 50.0f, 5.0f, 40.0f, 40.0f)];
    
//    self.activityImageView.layer.cornerRadius = 21.0f;
//    self.activityImageView.clipsToBounds = TRUE;
//
//    self.avatarImageView.layer.cornerRadius = 21.0f;
//    self.avatarImageView.clipsToBounds = TRUE;
    
    self.activityImageView.layer.cornerRadius = 5.0f;
    self.activityImageView.clipsToBounds = TRUE;
   ...
   // if (self.hasActivityImage) {
    ...
    CGSize contentSize = [self.contentLabel.text boundingRectWithSize:CGSizeMake([UIScreen mainScreen].bounds.size.width - 72.0f, CGFLOAT_MAX)
                                                    options:NSStringDrawingUsesLineFragmentOrigin // wordwrap?
                                                 attributes:@{NSFontAttributeName:[UIFont fontWithName:@"Montserrat-Regular" size:18.0f]}
                                                    context:nil].size;
    [self.contentLabel setFrame:CGRectMake( 66.0f, 16.0f, contentSize.width+2 - 40, contentSize.height)];
    CGRect nameButtonFrame = self.nameButton.frame;
    nameButtonFrame.origin.y = self.contentLabel.frame.origin.y;
    self.nameButton.frame = nameButtonFrame;
...
- (void)setActivity:(PFObject *)activity {
    ...
    if ([[activity objectForKey:kESActivityTypeKey] isEqualToString:kESActivityTypeFollow] || [[activity objectForKey:kESActivityTypeKey] isEqualToString:kESActivityTypeJoined] || [[activity objectForKey:kESActivityTypeKey] isEqualToString:kESActivityTypeMention]) {
       ...
       /* PFFile *file = (PFFile*)[[activity objectForKey:kESActivityPhotoKey] objectForKey:kESPhotoThumbnailKey];
        if (file) {
            [self setActivityImageFile:(PFFile*)[[activity objectForKey:kESActivityPhotoKey] objectForKey:kESPhotoPictureKey]];
        }
        else {
            [self setActivityImageFile:(PFFile*)[[activity objectForKey:kESActivityPhotoKey] objectForKey:@"videoThumbnailRound"]];
        }*/
        
         PFFile *file = (PFFile*)[[activity objectForKey:kESActivityPhotoKey] objectForKey:kESPhotoPictureKey];
         if (file) {
         [self setActivityImageFile:(PFFile*)[[activity objectForKey:kESActivityPhotoKey] objectForKey:kESPhotoPictureKey]];
         }
         else {
   ...
   /* if (![self.user objectForKey:kESUserProfilePicSmallKey]) {
    ...
    if (nameString.length > 0) {
        CGSize nameSize = [self.nameButton.titleLabel.text boundingRectWithSize:CGSizeMake(nameMaxWidth, CGFLOAT_MAX)
                                                                     ...
                                                                     attributes:@{NSFontAttributeName:[UIFont fontWithName:@"Montserrat-Bold" size:18.0f]}
                                                                        context:nil].size;
        NSString *paddedString = [ESBaseTextCell padString:activityString withFont:[UIFont fontWithName:@"Montserrat-Regular" size:18.0f] toWidth:nameSize.width];
        
        paddedString = [NSString stringWithFormat:@"%@ %@", self.nameButton.titleLabel.text, [paddedString stringByTrimmingCharactersInSet:NSCharacterSet.whitespaceCharacterSet]];
    ...
    self.nameButton.titleLabel.textColor = [UIColor clearColor];
}
...
+ (CGFloat)heightForCellWithName:(NSString *)name contentString:(NSString *)content cellInsetWidth:(CGFloat)cellInset {
    CGSize nameSize = [name boundingRectWithSize:CGSizeMake(200.0f, CGFLOAT_MAX)
                                                 ...
                                                 attributes:@{NSFontAttributeName:[UIFont fontWithName:@"Montserrat-Bold" size:18.0f]}
                                                    context:nil].size;
    NSString *paddedString = [ESBaseTextCell padString:content withFont:[UIFont fontWithName:@"Montserrat-Regular" size:18.0f] toWidth:nameSize.width];
    ...
    return fmax(50.0f + fmax(0.0f, multilineHeightAddition), 70);
}
