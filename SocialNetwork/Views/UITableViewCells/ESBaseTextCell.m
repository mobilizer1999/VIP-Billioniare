...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        [self.contentLabel setFont:[UIFont fontWithName:@"Montserrat-Regular" size:13.0f]];
        [self.contentLabel setTextColor:[UIColor colorWithWhite:0 alpha:0.7]];
        [self.contentLabel setTintColor:[UIColor colorWithWhite:0 alpha:0.8]];
        ...
        [self.nameButton setTitleColor:[UIColor colorWithWhite:0 alpha:1] forState:UIControlStateNormal];
        [self.nameButton setTitleColor:[UIColor colorWithWhite:0 alpha:0.7] forState:UIControlStateHighlighted];
        [self.nameButton.titleLabel setFont:[UIFont fontWithName:@"Montserrat-Regular" size:13]];
        ...
        [self.replyButton setTitleColor:def_Golden_Color forState:UIControlStateNormal];
        [self.replyButton setTitleColor:def_Golden_Color8 forState:UIControlStateHighlighted];
        [self.replyButton.titleLabel setFont:[UIFont fontWithName:@"Montserrat-light" size:10]];
        ...
        [mainView addSubview:self.grayView];
        self.timeLabel = [[UILabel alloc] init];
        [self.timeLabel setFont:[UIFont fontWithName:@"Montserrat-Regular" size:11]];
        [self.timeLabel setTextColor:[UIColor colorWithWhite:0.5 alpha:0.8]];
        [self.timeLabel setBackgroundColor:[UIColor clearColor]];
        [self.timeLabel setShadowColor:[UIColor colorWithWhite:1.0f alpha:0.70f]];
        [self.timeLabel setShadowOffset:CGSizeMake(0, 0.5f)];
        ...
        self.separatorImage = nil; //[[UIImageView alloc] initWithImage:[[UIImage imageWithColor:def_Golden_Color] resizableImageWithCapInsets:UIEdgeInsetsMake(0, 1, 0, 0)]];
...
- (void)layoutSubviews {
    ...
    CGSize nameSize = [self.nameButton.titleLabel.text boundingRectWithSize:CGSizeMake(nameMaxWidth, CGFLOAT_MAX)
                                                                 ...
                                                                 attributes:@{NSFontAttributeName:[UIFont fontWithName:@"Montserrat-Bold" size:13.0f]}
                                                                    context:nil].size;
    [self.nameButton setFrame:CGRectMake(nameX + 20, nameY, nameSize.width, nameSize.height)];
    
    // Layout the content
    CGSize contentSize = [self.contentLabel.text boundingRectWithSize:CGSizeMake(horizontalTextSpace, CGFLOAT_MAX)
                                                              options:NSStringDrawingUsesLineFragmentOrigin
                                                           attributes:@{NSFontAttributeName:[UIFont fontWithName:@"Montserrat-Regular" size:13.0f]}
                                                              context:nil].size;
    [self.contentLabel setFrame:CGRectMake(nameX + 20, 4 + nameSize.height + vertTextBorderSpacing, contentSize.width, contentSize.height)];
    ...
    CGSize timeSize = [self.timeLabel.text boundingRectWithSize:CGSizeMake(horizontalTextSpace, CGFLOAT_MAX)
                                                     ...
                                                     attributes:@{NSFontAttributeName:[UIFont fontWithName:@"Montserrat-Regular" size:13.0f]}
                                                        context:nil].size;
    self.grayView.frame = CGRectMake(timeX - 10 + 20, 0, SCR_W / 3 * 2, contentLabel.frame.origin.y + contentLabel.frame.size.height +  10 + vertElemSpacing * 2);
    
    if (_colorDotLayer == nil) {
        _colorDotLayer = [CAShapeLayer layer];
        _colorDotLayer.fillColor = [UIColor colorWithWhite:0.9 alpha:0.3].CGColor;
        [mainView.layer addSublayer:_colorDotLayer];
    }

    CGRect bounds = self.bounds;
    UIBezierPath *path = [UIBezierPath bezierPath];
    [path moveToPoint:CGPointMake(timeX-10, 40)];
    [path addLineToPoint:CGPointMake(timeX + 10, 20)];
    [path addLineToPoint:CGPointMake(timeX + 10, 40)];
    [path closePath];
    [path applyTransform:CGAffineTransformMakeTranslation(0,0)];
    _colorDotLayer.path = path.CGPath;
    
    
    
    
    [self.grayView setBackgroundColor:[UIColor colorWithWhite:0.9 alpha:0.3]];
    self.grayView.layer.cornerRadius = 15;
    
    if ([self isKindOfClass:[ESActivityCell class]])
        [self.grayView setHidden:YES];
    [self sendSubviewToBack:self.grayView];
    [self.timeLabel setFrame:CGRectMake(timeX + SCR_W / 3 * 2 - timeSize.width, contentLabel.frame.origin.y + contentLabel.frame.size.height + 2 * vertElemSpacing + 10, timeSize.width, timeSize.height)];
    ...
    // Layour separator
    [self.separatorImage setFrame:CGRectMake(15, self.frame.size.height-2, self.frame.size.width-cellInsetWidth*2 - 30, 0.5f)];
...
+ (CGFloat)heightForCellWithName:(NSString *)name contentString:(NSString *)content cellInsetWidth:(CGFloat)cellInset {
    ...
    NSString *paddedString = [ESBaseTextCell padString:content withFont:[UIFont fontWithName:@"Montserrat-Regular" size:13] toWidth:nameSize.width];
...
- (void)setUser:(PFUser *)aUser {
    ...
    if (![self.user objectForKey:kESUserProfilePicSmallKey]) {
        NSData* data = UIImageJPEGRepresentation([UIImage imageNamed:@"AvatarPlaceholder"], 0.5f);
        PFFile *imageFile = [PFFile fileWithName:@"AvatarPlaceholder" data:data];
...
- (void)setContentText:(NSString *)contentString {
    ...
    if (self.user) {
        self.nameButton.titleLabel.font = [UIFont fontWithName:@"Montserrat-Bold" size:13];
        CGSize nameSize = [self.nameButton.titleLabel.text boundingRectWithSize:CGSizeMake(nameMaxWidth, CGFLOAT_MAX)
                                                                        options:NSStringDrawingTruncatesLastVisibleLine|NSStringDrawingUsesLineFragmentOrigin
                                                                     attributes:@{NSFontAttributeName:[UIFont fontWithName:@"Montserrat-Bold" size:13.0f]}
                                                                        context:nil].size;
        NSString *paddedString = [ESBaseTextCell padString:contentString withFont:[UIFont fontWithName:@"Montserrat-Regular" size:13] toWidth:nameSize.width];
