...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self) {
        ...
        self.followButton.titleLabel.font = [UIFont fontWithName:@"Montserrat-Regular" size:15.0f];
        self.followButton.titleEdgeInsets = UIEdgeInsetsMake( 0.0f, 10.0f, 0.0f, 0.0f);
        [self.followButton setBackgroundImage:[UIImage imageNamed:@"follow.png"]
                                     forState:UIControlStateNormal];
        [self.followButton setBackgroundImage:[UIImage imageNamed:@"folowing.png"]
                                     forState:UIControlStateSelected];
        
//        [self.followButton setImage:[UIImage imageNamed:@"IconTick"]
//                           forState:UIControlStateSelected];
        [self.followButton setTitle:NSLocalizedString(@"Follow  ", @"Follow string, with spaces added for centering")
                           forState:UIControlStateNormal];
        [self.followButton setTitle:NSLocalizedString(@"Following  ", nil)
                           forState:UIControlStateSelected];
...
- (void)setUser:(PFUser *)aUser {
    ...
    if (nameString == nil) nameString = @"[Deleted User]";
    ...
    CGSize photoLabelSize = [@"photos" boundingRectWithSize:CGSizeMake([UIScreen mainScreen].bounds.size.width/2, CGFLOAT_MAX)
                                                 ...
                                                 attributes:@{NSFontAttributeName:[UIFont fontWithName:@"Montserrat-Regular" size:11.0f]}
                                                    context:nil].size;
    [photoLabel setFrame:CGRectMake( 60.0f, 20.0f + nameSize.height, 140.0f, photoLabelSize.height)];
