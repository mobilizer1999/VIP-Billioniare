...
@synthesize interaction, window, background, hud, spinner, image, label;
 ...
 {
     [self showSuccess:status sleepTimePerLetter:0.04];
}
...
+ (void)showSuccess:(NSString *)status sleepTimePerLetter:(CGFloat)sleepTimePerLetter
{
    [[self shared] hudMake:status image:HUD_IMAGE_SUCCESS spin:NO hide:YES sleepTimerPerLetter:sleepTimePerLetter];
}
 ...
 {
     [self hudMake:status image:img spin:spin hide:hide sleepTimerPerLetter:0.04];
}

- (void)hudMake:(NSString *)status image:(UIImage *)img spin:(BOOL)spin hide:(BOOL)hide sleepTimerPerLetter:(CGFloat)sleepTimePerLetter
 {
    [self hudCreate];
    label.text = status;
    ...
    [self hudShow];
    if (hide) [NSThread detachNewThreadSelector:@selector(timedHide:) toTarget:self withObject:[NSNumber numberWithFloat:sleepTimePerLetter]];
}
...
- (void)timedHide:(NSNumber *)sleepTimePerLetter
 {
	@autoreleasepool
	{
		double length = label.text.length;
		NSTimeInterval sleep = length * [sleepTimePerLetter floatValue] + 0.5;
