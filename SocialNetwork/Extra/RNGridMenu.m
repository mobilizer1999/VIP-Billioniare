...
CGFloat const kRNGridMenuDefaultDuration = 0.05f;
CGFloat const kRNGridMenuDefaultBlur = 0;
...
- (void)layoutSubviews {
    ...
    if (hasImage) {
        ...
        self.imageView.frame = CGRectInset(CGRectMake(CGRectGetWidth(frame) * 0.1f, y + CGRectGetWidth(frame) * 0.1f, CGRectGetWidth(frame) * 0.8f, 0.8f * height), inset, inset);
    }
    else {
...
- (instancetype)initWithItems:(NSArray *)items {
    if ((self = [super init])) {
        ...
        _itemTextColor = def_Golden_Color;//[UIColor whiteColor];
        _itemFont = [UIFont fontWithName:@"Montserrat-Bold" size:12.f];
        _highlightColor = def_Golden_Color8;//[UIColor colorWithRed:.02f green:.549f blue:.961f alpha:1.f];
...
- (void)layoutAsArc {
    ...
    CGPoint centerOfVIP = CGPointMake([UIScreen mainScreen].bounds.size.width / 2, [UIScreen mainScreen].bounds.size.height - 130);
    CGFloat radius = [UIScreen mainScreen].bounds.size.width / 2 * 0.8;
    
    NSUInteger listItemCount = self.itemViews.count;
    
    [self.itemViews enumerateObjectsUsingBlock:^(RNMenuItemView *itemView, NSUInteger idx, BOOL *stop) {
        itemView.frame = CGRectMake(0, 0, self.itemSize.width, self.itemSize.height);
        itemView.center = CGPointMake(centerOfVIP.x - cos(idx * 1.0 / (listItemCount - 1) * 3.141592) * radius, centerOfVIP.y - 0.6 * sin(idx * 1.0 / (listItemCount - 1) * 3.141592) * radius + (idx == 0 || idx == 4 ? 20 : 0));
    }];
}
- (void)layoutAsGrid {
...
- (void)showAnimated:(BOOL)animated {
    ...
    UIView* alphaBlackView = [[UIView alloc] initWithFrame: self.view.frame];
    alphaBlackView.backgroundColor = [UIColor colorWithWhite:0 alpha:0.8];
