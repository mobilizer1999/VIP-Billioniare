...
- (void)layoutSubviews {
    ...
    if (image != nil) {
        if (image.size.width >= image.size.height) {
            CGRect imageFrame = self.customImageView.frame;
            imageFrame.size.height = (SCR_W - 40) * (image.size.height / image.size.width);
            self.customImageView.frame = imageFrame;
        }
    }   
    
    
//    NSLog(@"%f, %f", _customImageView.frame.size.width, _customImageView.frame.size.height);
