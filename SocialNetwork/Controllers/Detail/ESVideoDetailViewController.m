...
-(void)viewWillDisappear:(BOOL)animated{
    ...
    [self.headerView.movie.player pause];
}
- (void)viewDidLoad {
...
//    self.tableView.backgroundColor = [UIColor colorWithWhite:0.90 alpha:1];
    ...
    CGSize labelSize = [[self.video objectForKey:@"photoDescription"] sizeWithFont:[UIFont fontWithName:@"Montserrat-Regular" size:15]
                                                                 constrainedToSize:CGSizeMake(self.view.frame.size.width-40, 20)
                                                                     lineBreakMode:NSLineBreakByWordWrapping];
    
    CGFloat labelHeight = labelSize.height;
    
    CGRect rect = [ESVideoDetailsHeaderView rectForView];
        
    self.headerView = [[ESVideoDetailsHeaderView alloc] initWithFrame:CGRectMake(rect.origin.x, rect.origin.y, rect.size.width, labelHeight + rect.size.height + 50) video:self.video];
...
- (void)activityButtonAction:(id)sender {
    if (NSClassFromString(@"UIActivityViewController")) {
        ...
        if ([[self.video objectForKey:kESVideoFileKey] isDataAvailable]) {
            [self showShareSheet];
        }
        else {
            [MBProgressHUD showHUDAddedTo:self.view animated:YES];
            [[self.video objectForKey:kESVideoFileKey] getDataInBackgroundWithBlock:^(NSData *data, NSError *error) {
...
- (void)showShareSheet {
        ...
        [[self.video objectForKey:kESVideoFileKey] getDataInBackgroundWithBlock:^(NSData *data, NSError *error) {
            if (!error) {
                ...
                if ([[[PFUser currentUser] objectId] isEqualToString:[[self.video objectForKey:kESPhotoUserKey] objectId]] && [self.objects count] > 0) {
                    PFObject *firstActivity = self.objects[0];
                    if ([[[firstActivity objectForKey:kESActivityFromUserKey] objectId] isEqualToString:[[self.video objectForKey:kESPhotoUserKey] objectId]]) {
...
-(void)editPost
{
    ...
    viewController.photoFile = [self.video objectForKey:kESVideoFileThumbnailKey];
    viewController.thumbnailFile = [self.video objectForKey:kESVideoFileThumbnailKey];
    viewController.descriptionPhoto = [self.video objectForKey:kESEditPhotoViewControllerDescriptionKey];
    viewController.editVideoObject = self.video;
