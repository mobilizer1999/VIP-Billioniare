...
//    self.view.backgroundColor = [UIColor colorWithRed:0.3412 green:0.6902 blue:0.9294 alpha:1];
    ...
    self.ivScreenImage.contentMode = UIViewContentModeScaleAspectFit; // shweta
    [self.view addSubview:self.ivScreenImage];
    [self.view addSubview:self.lblScreenLabel];
    self.lblScreenLabel.textColor = def_Golden_Color; // [UIColor whiteColor];
    ...
    self.lblScreenLabel.font = [UIFont fontWithName:@"Montserrat-Light" size:18];
    ...
    if (IS_IPHONE6) {
        ...
        self.ivScreenImage.frame = CGRectMake([UIScreen mainScreen].bounds.size.width/4.5, 140, [UIScreen mainScreen].bounds.size.width/1.8, [UIScreen mainScreen].bounds.size.height-250);
