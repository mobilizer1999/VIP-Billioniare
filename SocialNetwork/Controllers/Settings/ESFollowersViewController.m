...
@implementation ESFollowersViewController
@synthesize searchQuery, headerView;
@synthesize followStatus;
@synthesize selectedEmailAddress;
@synthesize outstandingFollowQueries;
@synthesize outstandingCountQueries, queriedUsers, user;
...
- (void)viewDidLoad {
    ...
    [self.searchController.view addSubview:self.searchResultsTableView ];
    self.searchResultsTableView.hidden = NO;
    [self.searchController.view bringSubviewToFront:self.searchController.searchBar];
    self.searchResultsTableView.separatorStyle = UITableViewCellSeparatorStyleNone;
    self.tableView.separatorStyle = UITableViewCellSeparatorStyleNone;
    [self.searchResultsTableView setFrame:self.tableView.frame];
    CGRect rt = self.searchResultsTableView.frame;
    rt.origin.y += self.searchController.searchBar.frame.size.height;
    self.tableView.bounces = NO;
    self.searchController.searchBar.barTintColor = [UIColor blackColor];
    [self.searchResultsTableView setFrame:rt];
    //Search friends
...
//    self.searchController.searchResultsTableView.backgroundColor = [UIColor blackColor];
...
//
    ...
    self.tableView.bounces = self.searchResultsTableView.bounces = NO;
...
- (void)objectsDidLoad:(NSError *)error {
    ...
    for (PFObject *object in self.objects) {
        if ([self.option isEqualToString:@"Following"]) {
            ...
            if (_user.objectId == nil) continue;            
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row < self.objects.count) {
        PFUser *_user;
        if ([self.option isEqualToString:@"Following"]) {
            _user = (PFUser *)[self.objects[indexPath.row] objectForKey:kESActivityToUserKey];
        }
        else {
            _user = (PFUser *)[self.objects[indexPath.row] objectForKey:kESActivityFromUserKey];
        }
        if (_user == nil) return 0;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath object:(PFObject *)object {
    if (tableView == self.tableView) {
        ...
        if ([self _shouldShowPaginationCell] && indexPath.row== self.objects.count) {
            ...
            if ([self.option isEqualToString:@"Following"]) {
                ...
                _user = (PFUser*)[object objectForKey:kESActivityToUserKey];
        ...
        NSString *uniqueIdentifier = [NSString stringWithFormat:@"peopleCell____%ld_%ld", indexPath.section, indexPath.row];
