...
- (void)viewDidLoad {
    [super viewDidLoad];
   // UINavigationItem *backBtn=[[UINavigationItem alloc] initWithTitle:@"back"];
    UIBarButtonItem *backBtn=[[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"Back", nil) style:UIBarButtonItemStylePlain target:self action:@selector(backButtonAction:)];
    UIBarButtonItem *doneBtn=[[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"Done", nil) style:UIBarButtonItemStylePlain target:self action:@selector(doneButtonAction:)];
    
    [backBtn setTitleTextAttributes:[NSDictionary dictionaryWithObjectsAndKeys:def_Golden_Color,  UITextAttributeTextColor,nil] forState:UIControlStateNormal];
    [doneBtn setTitleTextAttributes:[NSDictionary dictionaryWithObjectsAndKeys:def_Golden_Color,  UITextAttributeTextColor,nil] forState:UIControlStateNormal];
    ...
    self.navigationItem.title = NSLocalizedString(@"Blocked list", nil);
    ...
    [app.unblockUserArray removeAllObjects];
...
- (PFQuery *)queryForTable {
    ...
    [query whereKey:@"objectId" containedIn:arrayBlockUserIds];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath object:(PFObject *)object
{
    if (tableView == self.tableView)
    {
            ...
            {
                UnBlockCell *cell = [tableView dequeueReusableCellWithIdentifier:FriendCellIdentifier];
                
                if (cell == nil)
                {
                    cell = [[UnBlockCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:FriendCellIdentifier];
                ...
                [cell.photoLabel setText:NSLocalizedString(@"0 photos", nil)];
                 ...
                 [cell.unBlockButton addTarget:self action:@selector(didTapFollowButtonAction:) forControlEvents:UIControlEventTouchUpInside];
                [cell.unBlockButton setTag:indexPath.section];
                ...
                NSLog(@"%@",app.unblockUserArray);
                
                if ([app.unblockUserArray containsObject:(PFUser*)object]) {
                    
                    [cell.unBlockButton setSelected:NO];
                }
                else
                {
                    [cell.unBlockButton setSelected:YES];
                ...
                /*if (self.followStatus == ESFindFriendsFollowingSome)
                {
                    if (attributes)
                    {
                        [cell.unBlockButton setSelected:[[ESCache sharedCache] followStatusForUser:(PFUser *)object]];
                    }
                    else
                    {
                        @synchronized(self)
                        {
                            ...
                            if (!outstandingQuery)
                            {
                                ...
                                [isFollowingQuery countObjectsInBackgroundWithBlock:^(int number, NSError *error)
                                {
                                    ...
                                    {
                                        [cell.unBlockButton setSelected:(!error && number > 0)];
                                    }
                                }];
                            }
                        }
                ...
                {
                    [cell.unBlockButton setSelected:(self.followStatus == ESFindFriendsFollowingAll)];
                }*/ //shweta
...
- (void)doneButtonAction:(id)sender {
    ...
    [query whereKey:@"UserID" containedIn:app.unblockUserArray];
...
- (void)presentMailComposeViewController:(NSString *)recipient {
    ...
    //[composeEmailViewController setMessageBody:@"<h2>Share your pictures, share your story.</h2><p><a href=\"itms-apps://itunes.apple.com/app/id1076103571\">d'Netzwierk</a> is the easiest way to share photos with your friends. Get the app and share your fun photos with the world.</p><p><a href=\"itms-apps://itunes.apple.com/app/id1076103571\">d'Netzwierk</a> is fully powered by the luxembourgish developers Eric Schanet and Yannick Erpelding." isHTML:YES];
...
- (void)presentMessageComposeViewController:(NSString *)recipient {
   ...
   // [composeTextViewController setBody:NSLocalizedString(@"Check out d'Netzwierk! itms-apps://itunes.apple.com/app/id1076103571", nil)];
...
- (void)didTapFollowButtonAction:(UIButton*)sender {
    ...
    UnBlockCell *cell = [self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:sender.tag]];

    NSString * us= [cell.user objectId];
    ...
    if (sender.selected) {
        ...
        NSLog(@"block %@",app.unblockUserArray);
    ...
    else
    {
        ...
        [cell.unBlockButton setSelected:YES];
        NSLog(@"unblock %@",app.unblockUserArray);
