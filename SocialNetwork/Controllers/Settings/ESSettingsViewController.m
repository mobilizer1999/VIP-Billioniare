...
- (void)viewDidLoad {
    ...
    _tableView.separatorColor = [UIColor colorWithWhite:0 alpha:0.5];
    ...
    tableFooter.backgroundColor = [UIColor clearColor];//[self._tableView backgroundColor];
    tableFooter.opaque = YES;
    tableFooter.textAlignment = NSTextAlignmentCenter;
    tableFooter.font = [UIFont fontWithName:@"Montserrat-Regular" size:12];
    ...
    spaceInvader.titleLabel.font = [UIFont fontWithName:@"Montserrat-Regular" size:12];
    spaceInvader.backgroundColor = [UIColor clearColor];
...
- (UITableViewCell *)tableView:(UITableView *)aTableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        if (indexPath.row == 0) {
            ...
            cell.textLabel.text = NSLocalizedString(@"Blocked", nil);
            cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
  ////          cell.accessoryView = switchviewPush;
 //           [switchviewPush addTarget:self action:@selector(switchToggledPush) forControlEvents: UIControlEventValueChanged];
//            if([[[PFUser currentUser] objectForKey:@"pushnotification"] isEqualToString:@"YES"]){
  ...
  if (indexPath.section == 0) {
    ...
    else if (indexPath.section == 1)
    {
        ...
        else if (indexPath.row==5)
        {
            ...
            UnBlockViewcontroller *unblockViewController = [[UnBlockViewcontroller alloc] initWithNibName:nil bundle:nil];
            [self.navigationController pushViewController:unblockViewController animated:YES];
            ...
            if ([MFMailComposeViewController canSendMail]) {
               ...
               // [mailComposer setToRecipients:[NSArray arrayWithObjects: @"isso@vipbillionaires.com",nil]];
                [mailComposer setToRecipients:[NSArray arrayWithObjects: @"info@zedinternational.net",nil]];

                [mailComposer setSubject:[NSString stringWithFormat: @"v%@ Support",appVersionString]];
                NSString *supportText = [NSString stringWithFormat:@"Device: %@\niOS Version: %@\nRequester ID: %@\n               _________________",model,iOSVersion,[[PFUser currentUser]objectId]];
...
- (void)spaceInvaderTap {
    if (![[NSUserDefaults standardUserDefaults] boolForKey:@"spaceInvader1"]) {
        ...
        [alert showCustom:self.parentViewController image:[UIImage imageNamed:@"spaceInvader"] color:color title:@"Space Invader" subTitle:NSLocalizedString(@"You found one of the rare space invaders! Go and find the others to see what happens!", nil) closeButtonTitle:@"OK" duration:0.0];
