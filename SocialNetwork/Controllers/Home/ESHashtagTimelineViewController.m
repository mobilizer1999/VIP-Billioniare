//
//  ESHashtagTimelineViewController.m
//  d'Netzwierk
//
...
float previousAmount1;


@interface ESHashtagTimelineViewController () {
}
@end

@implementation ESHashtagTimelineViewController
@synthesize reusableSectionHeaderViews;
@synthesize reusableSectionFooterViews;
@synthesize shouldReloadOnAppear;
@synthesize outstandingSectionHeaderQueries;
@synthesize outstandingSectionFooterQueries;
@synthesize activityIndicator, tapOnce, tapTwice, hashtag;

#pragma mark - Initialization

- (id)initWithStyle:(UITableViewStyle)style andHashtag:(NSString *)haschischtag{
    self = [super initWithStyle:UITableViewStyleGrouped];
    if (self) {
        hashtag = haschischtag;
...
- (void)viewDidLoad {
    ...
    NSString *hashString = [[NSString alloc] initWithFormat:@"#%@",hashtag];
    self.navigationItem.title = hashString;
...
- (void)objectsDidLoad:(NSError *)error {
    ...
    if ([self.objects count] > 0) {
        ...
        UILabel *labelView = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 100)];
        labelView.font = [UIFont fontWithName:@"Montserrat-Regular" size:16];
        labelView.textColor = [UIColor darkGrayColor];
        labelView.textAlignment = NSTextAlignmentCenter;
        labelView.text = @"No posts";
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    if (scrollView.tag == 1) {
        ...
        CGFloat scrollDiff = scrollOffset - previousAmount1;
...
//        [self loadNextPage];
        ...
        if ([object objectForKey:@"type"] && [object objectForKey:kESVideoFileKey]) {
            ...
            if (cell.movie.player.timeControlStatus != AVPlayerTimeControlStatusPlaying) {
                
                cell.mediaItemButton.hidden = YES;
                cell.imageView.hidden = YES;
//                [cell.movie prepareToPlay];
                [cell.movie.player play];
...
- (PFQuery *)queryForTable {
    ...
    PFQuery *searchedHashtagQuery = [PFQuery queryWithClassName:kESActivityClassKey];
    [searchedHashtagQuery whereKey:@"hashtags" containsString:hashtag];
    [searchedHashtagQuery includeKey:kESActivityPhotoKey];
    [searchedHashtagQuery setCachePolicy:kPFCachePolicyNetworkOnly];
    
    PFQuery *photosFromSearchedHashtag = [PFQuery queryWithClassName:kESPhotoClassKey];
    [photosFromSearchedHashtag whereKey:kESUserObjectIdKey matchesKey:kESUserPhotoIDKey inQuery:searchedHashtagQuery];
    //[photosFromSearchedHashtag whereKeyExists:kESPhotoPictureKey];
    
    PFQuery *videosFromSearchedHashtag = [PFQuery queryWithClassName:self.parseClassName];
    [videosFromSearchedHashtag whereKey:kESUserObjectIdKey matchesKey:kESUserPhotoIDKey inQuery:searchedHashtagQuery];
    [videosFromSearchedHashtag whereKeyExists:@"type"];
    [videosFromSearchedHashtag whereKeyExists:kESVideoFileKey];
    
    PFQuery *query = [PFQuery orQueryWithSubqueries:[NSArray arrayWithObjects:photosFromSearchedHashtag,videosFromSearchedHashtag, nil]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath object:(PFObject *)object {
    ...
    if ([self _shouldShowPaginationCell] && indexPath.section == self.objects.count) {
        ...
        cell.imageView.image = [UIImage imageNamed:@"PlaceholderPhoto1"];
        // cell.imageView.hidden= YES;
        if (object) {
        ...
        if (object) {
            CGSize labelSize = [[object objectForKey:@"text"] sizeWithFont:[UIFont fontWithName:@"Montserrat-Regular" size:16]
                                                         constrainedToSize:CGSizeMake([UIScreen mainScreen].bounds.size.width - 20, 100)
                                                             lineBreakMode:NSLineBreakByWordWrapping];
            CGFloat labelHeight = labelSize.height;
            cell.postText.frame = CGRectMake(25, 0, [UIScreen mainScreen].bounds.size.width-20, labelHeight+10);
