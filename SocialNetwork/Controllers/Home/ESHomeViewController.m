...
- (void)viewDidLoad
{
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"searchIcon"] style:UIBarButtonItemStylePlain target:self action:@selector(newHashtagSearch)];
    ...
    UILabel *label =[[UILabel alloc] initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width/2 - 253/2, 96.0f, 253.0f, 165.0f)];
    label.textColor = [UIColor blackColor];
    NSString * strWelcom = NSLocalizedString(@"You actually need friends to see a newsfeed.", nil);
    ...
    self.view.backgroundColor = def_Golden_Color5;
...
- (void)easterButtonTap {
    if (monster == NO) {
        ...
        [titleLabelView setFont:[UIFont fontWithName:@"Montserrat-Regular" size: 27]]; //<<--- Greatest font size
...
- (void)useNotificationWithString:(NSNotification *)notification //use notification method and logic
{
    ...
    else if ([stringValueToUse isEqualToString:@"FindFriendsOpen"])
    {
        ...
        findFriendsVC.hidesBottomBarWhenPushed = true;
        ...
        popularView.hidesBottomBarWhenPushed = true;
...
//            DZNWebViewController *webViewController = [[DZNWebViewController alloc] initWithURL:[NSURL URLWithString:@"http://www.google.com"]];
