...
- (void)viewDidLoad {
    ...
    if (self.isFromGroupInfoView == TRUE) {
        
        [selection addObjectsFromArray:self.arrExistimgMemberList];
    }
    ...
    else
    {
        if (self.isFromGroupInfoView == TRUE) {
            ...
            [self dismissViewControllerAnimated:YES completion:^{
                if (delegate != nil) {
                    ...
                    [delegate selectedRecipients:selectedUsers groupName:@"" image:nil];
                }
            }];
        }
        else
        {
            if ([selection count] > 1) {
                ...
                objAddGroupViewController.isEdit = FALSE;
...
- (void)selecteGroupNameIcon:(NSString *)groupName image:(UIImage*)image
{
    [self dismissViewControllerAnimated:YES completion:^{
        if (delegate != nil) {
            ...
            [delegate selectedRecipients:selectedUsers groupName:groupName image:image];
        }
    }];
}
...
- (void)loadUsers {
   ...
   // [query orderByAscending:@"createdAt"]; shweta created
