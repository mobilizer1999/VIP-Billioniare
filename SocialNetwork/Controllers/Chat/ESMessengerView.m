...
- (void)viewDidLoad {
    ...
    if ([groupId length] == 20) {
        ...
        else self.title = username;
    } else 	{
        self.title = username;
        
            UIButton *infoButton = [UIButton buttonWithType:UIButtonTypeInfoLight];
            [infoButton addTarget:self action:@selector(showGroupInfo) forControlEvents:UIControlEventTouchUpInside];
    ...
    placeholderProfilePicture = [JSQMessagesAvatarImageFactory avatarImageWithImage:[UIImage imageNamed:@"AvatarPlaceholderProfile"] diameter:30.0];
    ...
    [self.collectionView setBackgroundView:[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"background_splash"]]];
}
-(void)showGroupInfo
{
    GroupInfoTableViewController *objGroupInfoTableViewController = [[GroupInfoTableViewController alloc] init];
    objGroupInfoTableViewController.groupMemberList = [[NSMutableArray alloc] init];
    objGroupInfoTableViewController.groupMemberList = self.groupMemberList;
    objGroupInfoTableViewController.groupId = groupId;
    objGroupInfoTableViewController.groupTitle = username;
    objGroupInfoTableViewController.groupIcon = self.groupIcon;
    objGroupInfoTableViewController.recentId = self.recentId;
    objGroupInfoTableViewController.groupAdminId = self.groupAdminId;

    [self.navigationController pushViewController:objGroupInfoTableViewController animated:YES];
}
...
//    DZNWebViewController *webViewController = [[DZNWebViewController alloc] initWithURL:[dict objectForKey:@"url"]];
//    UINavigationController *navVC = [[UINavigationController alloc] initWithRootViewController:webViewController];
//    [self presentViewController:navVC animated:YES completion:NULL];
    [[UIApplication sharedApplication] openURL:[dict objectForKey:@"url"] options:@{} completionHandler:nil];
}
...
- (BOOL)addNewMessage:(NSDictionary *)messengerItem {
    ...
    if ([[messengerItem objectForKey:@"type"] isEqualToString:@"picture"]) {
        JSQPhotoMediaItem *photoItem = [[JSQPhotoMediaItem alloc] initWithImage:nil];  // shweta image
	...
	[query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error) {
		if (error == nil) {
			if ([objects count] != 0) {
                ...
                PFFile *file;
                file = [user objectForKey:kESUserProfilePicSmallKey];
                ...
                {
                    file = [user objectForKey:kESUserProfilePicMediumKey];
...
- (void)sendMessage:(NSString *)text withPicture:(UIImage *)picture withVideo:(NSURL *)video andWithAudio:(NSString *)audio {
    ...
    if (text != nil) {
        ...
        PFFile *file = [PFFile fileWithName:@"picture.jpg" data:UIImageJPEGRepresentation(picture, 0.5)];
...
- (void)eventuallySaveMessage:(NSMutableDictionary *)messengerItem {
    ...
    [ESUtility setDeliveredForMessage:groupId andString:@"Delivered"];
...
- (void)updateFirebaseMessage:(NSDictionary *)messengerItem {
    ...
    if ([defaults boolForKey:@"readreceipt"]) {
        if ([messengerItem[@"status"] isEqualToString:@"Read"] == NO)
        {
            ...
            [ESUtility setReadForMessage:groupId andString:@"Read"];
        }
    }
...
//    [self sendMessage:textView.text withPicture:nil withVideo:nil andWithAudio:nil];
//    return YES;
//}
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text {
    ...
    if([text isEqualToString:@"\n"]) {
        [UIView animateWithDuration:0.1 animations:^{
            [self sendMessage:[textView.text stringByTrimmingCharactersInSet:NSCharacterSet.whitespaceAndNewlineCharacterSet] withPicture:nil withVideo:nil andWithAudio:nil];
...
- (void)didPressSendButton:(UIButton *)button withMessageText:(NSString *)text senderId:(NSString *)senderId senderDisplayName:(NSString *)name date:(NSDate *)date {
    ...
    if ([text length] == 0 && originText.length > 0) return;
...
- (void)didPressSendButton1:(UIButton *)button withMessageText:(NSString *)text senderId:(NSString *)senderId senderDisplayName:(NSString *)name date:(NSDate *)date {
    [ESUtility shouldPresentPhotoCamera:self editable:NO];
//    [ESUtility shouldPresentPhotoLibrary:self editable:NO];
}

- (void)didPressSendButton2:(UIButton *)button withMessageText:(NSString *)text senderId:(NSString *)senderId senderDisplayName:(NSString *)name date:(NSDate *)date {
    [ESUtility shouldPresentPhotoLibrary:self editable:YES];
}
...
//    [action showInView:self.view];
    [ESUtility shouldPresentPhotoAndVideoCamera:self editable:NO];
}
...
- (NSAttributedString *)collectionView:(JSQMessagesCollectionView *)collectionView attributedTextForCellBottomLabelAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([message.senderId isEqualToString:self.senderId] == YES) {
		...
		return [[NSAttributedString alloc] initWithString:NSLocalizedString([messengerItem objectForKey:@"status"], nil)];
	}
	else return nil;
}
    ...
    [userQuery getObjectInBackgroundWithId:messageItem.senderId block:^(PFObject *object, NSError *error){
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (!error) {
                ...
                if (tappedUser != nil && ![[tappedUser objectId] isEqualToString:[[PFUser currentUser] objectId]]) {
...
//	}
  	if (message.isMediaMessage) {
		if ([message.media isKindOfClass:[JSQPhotoMediaItem class]]) {
            ...
            imageInfo.referenceContentMode = UIViewContentModeScaleAspectFit; // shweta image
            ...
            imageViewer.modalPresentationStyle = UIModalPresentationCustom;
...
//            [moviePlayer.moviePlayer play];
            ...
            NSURL *movieUrl = videoItem.fileURL;
...
//            [moviePlayer.moviePlayer play];
            AVPlayerViewController* movie = [[AVPlayerViewController alloc] init];
            NSURL *movieUrl = audioItem.fileURL;
            AVPlayer *player = [AVPlayer playerWithURL:movieUrl];
            movie.player = player;
		}
    }
